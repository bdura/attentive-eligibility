...
IDX_TO_COLOR = dict(zip(COLOR_TO_IDX_.values(), COLOR_TO_IDX_.keys()))


class PerturbedColor:

    def __getitem__(self, item):
        if isinstance(item, str):
            return COLOR_TO_IDX_[item]
        elif isinstance(item, tuple):
            return COLOR_TO_IDX_[item[0]] + item[1]
        print(item)

    def __contains__(self, item):
        return item in COLOR_TO_IDX_


COLOR_TO_IDX = PerturbedColor()
...
class RandomWall(WorldObj):
    def __init__(self, color='grey', noise=1):
        super().__init__('wall', color)
        perturbation_color = np.random.randn() * noise
        self.color = self.color, perturbation_color
...
class Grid:
    ...
    def horz_wall(self, x, y, length=None, noise=None):
        if length is None:
            length = self.width - x
        for i in range(0, length):
            w = RandomWall(noise=noise) if noise else Wall()
            self.set(x + i, y, w)

    def vert_wall(self, x, y, length=None, noise=None):
        if length is None:
    ...
    def wall_rect(self, x, y, w, h, noise):
        self.horz_wall(x, y, w, noise=noise)
        self.horz_wall(x, y + h - 1, w, noise=noise)
        self.vert_wall(x, y, h, noise=noise)
        self.vert_wall(x + w - 1, y, h, noise=noise)
